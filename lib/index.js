...
const { decode: _decode } = require('jsonwebtoken');

function defaultDecode(token) {
		return _decode(token, { complete: true });
}

module.exports = (opts = {}) => {
    const { debug, getToken, isRevoked, key = 'user', passthrough, tokenKey, verify = _verify, decode = defaultDecode } = opts;
    ...
    const middleware = async function jwt(ctx, next) {
        ...
        try {
            if (typeof secret === 'function') {
                secret = await getSecret(secret, token, decode);
            }
